...
const ASSETS = {
  ...
  modelHero: "https://files.manuscdn.com/user_upload_by_module/session_file/97386008/rCVCwrlAgXmqeHNk.png",
...
export async function createGameScene(canvas: HTMLCanvasElement, onSnapshot: (snapshot: GameSnapshot) => void, onPopup?: (ev: PopupEvent) => void): Promise<GameHandle> {
  ...
  modelSprite.parent = playerRoot; modelSprite.position.set(0, -0.2, -1.15); modelSprite.scaling.setAll(1.16); modelSprite.isVisible = false;
  const modelSpriteMat = new StandardMaterial("model-realistic-hero-material", scene);
  modelSpriteMat.diffuseColor = new Color3(0.16, 0.22, 0.38);
  modelSpriteMat.emissiveColor = new Color3(0.04, 0.06, 0.12);
  ...
  if (ENABLE_GLB) SceneLoader.ImportMeshAsync("", glbRootUrl, glbFileName, scene).then((result) => {
    ...
    // Keep the procedural mesh character visible; never expose a missing-texture checkerboard.
    glbReady = false;
    modelSprite.isVisible = false;
  });
  let bagAnimT = 0;
  ...
  const STAGE_SPECS: StageSpec[] = [
    {
      mode: "runway",
      platforms: [[-15,-3.06,280],[8,-1.2,8],[27,0.2,7],[48,-0.8,9],[72,0.7,8],[96,-0.4,10],[123,1.1,8],[151,-0.7,9],[181,0.8,9],[214,-0.2,12]],
      enemies: [[20,-1.6,"hedgehog"],[36,1.1,"bat"],[55,-1.4,"crow"],[78,1.3,"bat"],[103,-1.4,"hedgehog"],[128,1.2,"crow"],[158,-1.2,"bat"],[188,1.0,"hedgehog"],[219,-1.3,"crow"],[238,-1.4,"crow",true]],
      coins: [[6,-1.3],[12,.2],[25,1.1],[42,-.3],[58,1.4],[76,-.1],[100,1.1],[124,.4],[148,1.4],[175,.2],[204,1.2],[232,-.2]],
      items: [[14,-.6,"bag"],[62,1.2,"shoe"],[118,1.8,"wings"],[188,1.4,"bag"]], allies: [[38,-.7],[109,-.7],[177,-.7]],
    },
    {
      mode: "vertical",
      platforms: [[-15,-3.06,280],[12,-1.2,7],[27,1.0,6],[42,3.3,7],[60,1.8,6],[77,4.4,7],[97,6.8,8],[121,4.5,7],[145,7.8,8],[174,5.6,9],[205,8.2,12]],
      enemies: [[23,1.8,"bat"],[39,4.1,"crow"],[59,2.5,"bat"],[82,5.2,"hedgehog"],[101,7.5,"crow"],[128,5.1,"bat"],[151,8.5,"crow"],[181,6.4,"bat"],[216,9.0,"crow",true]],
      coins: [[14,-.4],[29,1.8],[45,4.2],[63,2.7],[80,5.3],[100,7.7],[124,5.4],[149,8.8],[178,6.7],[207,9.5]],
      items: [[31,2.2,"shoe"],[79,5.8,"bag"],[126,6.2,"wings"],[181,7.4,"shoe"]], allies: [[28,1.0],[79,4.4],[147,7.8]],
    },
    {
      mode: "underground",
  ...
  const STAGE_PROFILES: StageProfile[] = [
    ...
    { name: "SKYLINE ASCENT", mode: "vertical", boss: "Madame Elevator", gimmick: "sky-rails", terrainShift: 0.35, terrainWave: 0.28, accent: "cobalt" },
  ...
  const move = (direction: -1 | 1) => {
    ...
    lastInputDirection = direction; vx += direction * 4.6;
  };
  ...
  window.addEventListener("frw:start", jump); window.addEventListener("frw:restart", restartEvent); window.addEventListener("frw:next-stage", nextStageEvent);
  ...
  const update = (dt: number) => {
    ...
    vx *= Math.pow(0.0007, dt);
    const baseVx = crouching ? 1.4 : 2.4;
    const maxVx = speedBoostTimer > 0 ? 18.5 : 15.5;
    ...
    const gravityScale = hasWings && !lastLanded && jumpHeld ? 0.18 : 1.0;
    ...
    if (x >= 249.2 && !finished) {
      ...
      if (collectionIndex < TOTAL_STAGE_SECTIONS - 1) {
        autoAdvanceTimer = window.setTimeout(() => { autoAdvanceTimer = undefined; if (finished) nextStageEvent(); }, 1800);
      }
    }
    if (landed) {
    ...
    const targetLean = webAnchorIndex >= 0 ? Math.max(-0.34, Math.min(0.34, vx * 0.018)) : Math.max(-0.16, Math.min(0.16, vx * 0.008));
    renderLean += (targetLean - renderLean) * Math.min(1, dt * 12);
    const targetBob = lastLanded ? Math.sin(runCycle * 0.5) * 0.035 : Math.sin(globalT * 7) * 0.018;
    renderBob += (targetBob - renderBob) * Math.min(1, dt * 14);
    playerRoot.rotation.z += renderLean;
    playerRoot.position.y += renderBob;
    ...
    const heroMotion = Math.sin(runCycle * 0.5) * 0.018 + (flying ? -0.055 : 0);
    const requestedGLBAnimation = flying ? "Fly" : (bagAnimT > 0 || shoeAnimT > 0) ? "Attack" : Math.abs(vx) > 0.7 ? "Run" : "Idle";
